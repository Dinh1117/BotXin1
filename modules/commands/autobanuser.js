...
module.exports. run = ({api, event}) => {
  return api.sendMessage("Tự động ban người dùng nếu spam 8 lệnh / 2 phút", event.threadID, event.messageID);
};
...
module.exports.handleEvent = async ({ Users, api, event}) => {
  ...
  if ((global.client.autoban[senderID].timeStart + thoi_gian_spam) <= Date.now()) {
    ...
    if (global.client.autoban[senderID].number >= so_lan_spam) {
      ...
      api.sendMessage({
        body: senderID + " | " + dataUser.name + `\n🤍 🌸Bạn đã bị cấm sử dụng bot với lý do: spam bot ${so_lan_spam} lần/ ${Math.floor(thoi_gian_spam/60000)} phút \n🌸 Auto unban sau ${Math.floor(unbanAfter/60000)}phút`,
        ...
        api.sendMessage("🌸Phạm Nhân  " + senderID + " | " + dataUser.name + ` đã bị ban vì spam  ${so_lan_spam} lần/ ${Math.floor(thoi_gian_spam/60000)}phút \n 🌸Sẽ gỡ ban sau ${Math.floor(unbanAfter/60000)}phút\nThời gian: ` + time, idAdmin);
      };
    }
  }
};
